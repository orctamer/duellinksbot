...
app.post('/pay', (req, res) => {
    ...
    const create_payment_json = {
        ...
        "transactions": [{
            ...
            "description": `${amount}x Duel Links Meta Anytime Tickets For ${user}#${discrim} | Discord Id: ${id} | Email: ${email}`
        }]
    }
...
app.get('/member', checkAuth, async function (req, res) {
    let [person] = await Users.findOrCreate({
        where: { userid: req.user.id, username: req.user.username, email: req.user.email }
    }).all()
    res.render('member', { user: req.user.username, id: req.user.id, email: req.user.email, discrim: req.user.discriminator, avatar: req.user.avatar, tickets: person.tickets })
})
...
/* client.on('message', async msg => {
    ...
    if (msg.content === '!tickets') {
        msg.reply(`You have ${person.tickets === null ? '0' : person.tickets} ${person.tickets === 1 ? 'Ticket' : 'Tickets'}`)
    }
    if (msg.content === '!redeem') {
        if (person.tickets === 0 || !person.tickets) {
            return msg.reply(`Sorry, you don't have any tickets :( please buy some at [url to site]`)
        }
        person.update({
        ...
        msg.reply(`You have successfully redeemed 1 ticket! You have ${person.tickets} left!`)
         if (!currentTournament || currentTournament.state === 'complete' || currentTournament.state === 'underway')
         {
           return tournament.tournaments.create({
                tournament: {
                  name: `anytime_tournament_${number}`,
                  url: `anytime_tournament_${number}`,
        ...
        await tournament.participants.create({
            ...
            callback: (err, data) => {
              msg.reply(`Good Luck & Have Fun :D ${person.username}! Tournament located at : ${currentTournament.tournament.fullChallongeUrl}`)
            }
          })
    }
}) */
...
async function sendPayment(payment) {
    ...
    channel.send(`**SUCCESS**: :money_with_wings: Payment recieved from <@!${id}> \`${info.first_name}\` | ${user}#${discrim} E-mail: \`${info.email}\` | ${email} for ${amount.total} ${amount.currency} for ${quantity} ${quantity > 1 ? 'tickets' : 'ticket'}, this user has **${person.tickets}** now.`)
    author.send(`Thank you so much for your purchase of ${quantity} ${quantity > 1 ? 'tickets' : 'ticket'} you now have **${person.tickets}** :) `)
}
